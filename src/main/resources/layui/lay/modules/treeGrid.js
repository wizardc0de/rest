                ...
                treeDom: function (trs, rows) {
                    ...
                    $.each(trs, function (_, tr) {
                        ...
                        if (tr.next('.treegrid-parent-' + id).length > 0) {
                            $(EXPANDER).on('click', HELPER.onExpand).prependTo(tr.addClass('treegrid-expanded').find('td:eq(1)'));
                        } else if (grid.loadRow && grid.pageData.rows[tr.index()]._children) {
                            $(EXPANDER).on('click', HELPER.onLoadRow).prependTo(tr.addClass('treegrid-collapse').find('td:eq(1)'));
                        } else {
                            tr.find('td:eq(1)').prepend(INDENT);
                        }
                        if (grid.elem.find('tr.treegrid-' + pid).length > 0) {
                            ...
                            tr.find('td:eq(1)').prepend(pindent + INDENT);
                        }
                    });
                    grid.resize();
                }
